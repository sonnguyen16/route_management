...
<template>
  <MainLayout>
    <div class="py-3 px-4">
          ...
          <tbody>
            <template v-for="(it, i) in giam_sat.data" :key="i">
              <tr v-if="!it.giam_sat_id">
                ...
                <td></td>
                <td>
                  <a href="#">{{ it.don_vi ? it.don_vi.ten : '' }}</a>
                </td>
                <td>
              ...
              <tr v-for="(item, i) in it.doan_duong" :key="i">
                ...
                <td>{{ item.vi_tri }}</td>
                <td>
                  <span v-if="item.muc_do !== null"></span>{{ item.muc_do == 0 ? 'Nghiêm trọng' : 'X Nghiêm trọng' }}
                </td>
                <td>
                  <span v-if="item.tinh_trang_khac_phuc !== null">{{
                    item.tinh_trang_khac_phuc == 0 ? 'Khắc phục' : 'Chưa khắc phục'
                  }}</span>
                </td>
                <td></td>
      ...
      <Pagination :all-data="giam_sat" @changePage="changePage" />
    </div>
    <Modal
      ...
      :flag="flag"
    />
    <Question @close-modal="question.hideModal" @drop="drop" :obj="giam_sat_selected" :msg="msg" />
  </MainLayout>
</template>
