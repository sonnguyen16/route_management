...
<template>
  <div v-if="isSheetOpen" class="sheet">
    ...
    <div class="tabs">
      <button :class="{ active: currentTab === 'info' }" @click="currentTab = 'info'">Thông tin</button>
      <button :class="{ active: currentTab === 'documents' }" @click="currentTab = 'documents'">Tài liệu</button>
    </div>
    ...
    <div class="sheet-content px-2">
      ...
      <div v-if="currentTab === 'documents'">
        ...
        <ul v-if="selectedRoute.tai_lieu && selectedRoute.tai_lieu.length > 0">
          <File :file="file" v-for="file in selectedRoute.tai_lieu" :key="file.id" :enable-delete="false" />
        </ul>
