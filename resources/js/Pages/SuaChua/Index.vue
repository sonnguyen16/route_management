...
<template>
  <MainLayout>
    <div class="py-3 px-4">
          ...
          <tbody>
            <template v-for="(it, i) in sua_chua.data" :key="i">
              <tr v-if="!it.sua_chua_id">
                ...
                <td style="vertical-align: unset !important">
                  <label
                    ...
                    @click.prevent="chooseFile(it.id)"
                    class="cursor-pointer border-0 w-full text-start rounded-md mb-0"
                  >
                    <i class="fa fa-paperclip mr-2"></i>
                  </label>
                  <Upload :listFile="it.tai_lieu" @refresh="onRefresh" />
                </td>
                <td class="text-center">
                  <a @click.prevent="editModal(it)" class="edit cursor-pointer" title="Sửa"
                    ><i class="fas fa-edit mr-2"></i
                  ></a>
                  <a @click.prevent="remove(it)" class="edit cursor-pointer" title="Xóa"
              ...
              <tr v-for="(item, i) in it.doan_duong" :key="i">
                ...
                <td class="text-center">
                  <span v-if="item.ngay_duyet">{{ moment(item.ngay_duyet).format('DD/MM/YYYY') }}</span>
                </td>
                <td class="text-center">
                  <span v-if="item.ngay_khoi_cong">{{ moment(item.ngay_khoi_cong).format('DD/MM/YYYY') }}</span>
                </td>
                <td class="text-center">
                  <span v-if="item.ngay_hoan_thanh">{{ moment(item.ngay_hoan_thanh).format('DD/MM/YYYY') }}</span>
                </td>
                ...
                <td style="vertical-align: unset !important">
                  <label
                    style="font-weight: normal; color: #007bff"
                    @click.prevent="chooseFile(item.id)"
                    class="cursor-pointer border-0 w-full text-start rounded-md mb-0"
                  >
                    <i class="fa fa-paperclip mr-2"></i>
                  </label>
                  <Upload :listFile="item.tai_lieu" @refresh="onRefresh" />
                </td>

                <td class="text-center">
                  <a @click.prevent="editModal(item)" class="edit cursor-pointer" title="Sửa"
                    ><i class="fas fa-edit mr-2"></i
                  ></a>
                  <a @click.prevent="remove(item)" class="edit cursor-pointer" title="Xóa"
                    ><i class="fas fa-times-circle mr-1"></i
                  ></a>
                </td>
              </tr>
              ...
              <tr v-if="!it.sua_chua_id">
                ...
                <td>
                  <a @click.prevent="openModal(it)" class="edit cursor-pointer" title="Thêm đoạn đường"
      ...
      <Pagination :all-data="sua_chua" @changePage="changePage" />
    </div>
    <Modal
      ...
      :flag="flag"
    />
    <Question @close-modal="question.hideModal" @drop="drop" :obj="sua_chua_selected" :msg="msg" />
